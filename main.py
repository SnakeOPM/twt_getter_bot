...
def reply_on_button(messege):
    ...
    if messege.text == "Свиня":
        bot.send_animation(messege.chat.id, filename)
    else:
        get_tweet_by_hash(messege)
...
def get_tweet_by_hash(messege):
    ...
    i = 0
    for item in result:
        bot.send_message(messege.chat.id, f'Автор {result["username"][i]}\n пост {result["content"][i]}')
        try:
...
def redirect_to_hash(messege):
    msg = bot.reply_to(messege, 'Введите тег (Без #)')
    bot.register_callback_query_handler(msg, get_tweet_by_hash)
